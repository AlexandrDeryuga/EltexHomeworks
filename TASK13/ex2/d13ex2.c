...
char txtBuf[SYSBUF];
   ...
   else
   {
      ...
      if(RoomsCount<100){
         ...
         sscanf(buf.mText,"%d %s",&PidClient, str);
          ...
          int kk=Rooms[0].msg_counter;
         //AddNewMessage(&Rooms[0],buf.mText,1,NULL,0);  // вид сообщения [ PID имя_клиента ]    приват - это на будушее  для p2p комнат Клиент-server-Клиент
         AddNewMessage(&Rooms[RoomsCount-1],buf.mText,1,str,0); // -конструктор!
        //  Rooms[0].msg_counter=kk;
         Rooms[RoomsCount-1].id=PidClient;
        // strcpy(Rooms[RoomsCount-1].Name,str);//после того как конструктор отработал внесем id и имя
    ...
    //сделаем это через мета команду  в процесс 2 [addm кому сообщение]
     ...
     msg.mType=1;
     sprintf(msg.mText,"addm %d", Rooms[RoomsCount-1].id);// отправка последнему добавленному ну и по идее это PidClient
  ...
  sleep(0.3);  
   ...
   //pthread_mutex_unlock( &mutex );
      ...
      if ( msgctl( d->qId1, IPC_RMID, &msgCtlBuf ) == -1 ) {             //удаляем очередь
...
char txtBuf[SYSBUF];
   ...
   else
   {
      ...
      //pthread_mutex_lock( &mutex );  //add2
       ...
       s=strtok(NULL," ");
        ...
        id=atol(s);
         wprintw(logw,"SEVER th2>id=%ld,msg_counter=%d\n",id,Rooms[0].msg_counter);wrefresh(logw);
        i=0;
        pthread_mutex_lock( &mutex );  //add2
 ...
 while (i<Rooms[0].msg_counter){ 
         ...
         wprintw(logw,"Server th2 addm to[%ld]>%s\n",msg.mType,msg.mText);wrefresh(logw);sleep(1);
    if ( msgsnd( d->qId2, &msg, sizeof (msg.mText), IPC_NOWAIT ) == -1 ) {
                                                                          perror( "server: msgsnd failed:" );
                                                                          exit( 3 );
                                                                          } else{   };
     ...
     for(int ii=0;ii<RoomsCount;ii++){
      ...
      //wprintw(logw,"th2 msgsnd to[%ld]>%s\n",msg.mType,msg.mText);wrefresh(logw);
       sprintf(msg.mText,"0 %s", (Rooms[0].ptr+Rooms[0].Messages[Rooms[0].msg_counter-1])); //адрес + смещение для последнего сообщения в комнате 
       wprintw(logw,"th2 msgsnd to[%ld]>%s\n",msg.mType,msg.mText);wrefresh(logw);
         if ( msgsnd( d->qId2, &msg, sizeof msg.mText, IPC_NOWAIT ) == -1 ) {
                                                                          perror( "server: msgsnd failed:" );
                                                                          exit( 3 );
   ...
   continue;
      ...
      pthread_mutex_lock( &mutex );
        ...
        sscanf(buf.mText,"%d %d",&idfrom, &id);
        //  wprintw(logw,"%d %d %s\n",idfrom, id, buf.mText);
         char *x;
        // if(!id) id=1;
       ...
       //  for(i=0;i<RoomsCount;i++) 
           if(id) {                                            //добавить сообщение в нужную комнату преобразовав его
              ...
              if(-1!=AddNewMessage(&Rooms[i],msg.mText,0,NULL,0)) {// если успешно добавили то отправка последнего сообщения из комнаты
                                                                           msg.mType=id;
                                                                      ...
                                                                      for(int ii=1;ii<RoomsCount;ii++){     //разошлнм всем КЛИЕНТАМ последнее            
                                                                          ...
                                                                          // блок отсылки
                                                                          msg.mType=Rooms[ii].id;
                                                                          sprintf(msg.mText,"%ld %s",idfrom,Rooms[i].ptr+Rooms[i].Messages[Rooms[i].msg_counter-1]);
                                                                           wprintw(logw,"th2 reposted: to[%ld]>%s\n",msg.mType,msg.mText);wrefresh(logw); 
                                                                          if ( msgsnd( d->qId2, &msg, sizeof msg.mText, IPC_NOWAIT ) == -1 ) {
                                                                          perror( "server: msgsnd failed:" );
                                                                          exit( 3 );
                                                                          } else{   };
 ...
 if (msgctl(d->qId2, IPC_RMID, &msgCtlBuf ) == -1 ) {             //удаляем очередь
      perror( "server: msgctl failed:" );
      exit( 4 );
   }
...
ML_data.qId1=qId_th1;ML_data.qId2=qId_th2;
...
if(ch==pushmessage) {
            sprintf(msg.mText,"sends_msg");msg.mType=1;
               wprintw(wtxt3,"SERVER menu push message\n");wrefresh(wtxt3);
            if ( msgsnd( qId_th2, &msg, sizeof msg.mText,IPC_NOWAIT ) == -1 ) {                             // посылаем сигнал на выход нашему потоку обработчика сообщений
            perror( "client: msgsnd failed:" );
...
// завершение работы
  ...
  msg.mType = 1;                
     ...
     sprintf(msg.mText,"exit_now");
            if ( msgsnd( qId_th1, &msg, sizeof msg.mText, IPC_NOWAIT ) == -1 ) {                             // посылаем сигнал на выход нашему потоку SystemMsg
            perror( "client: msgsnd failed:" );
      // exit( 3 ); //ноувайт воде бы в ошибку должен выпадать,поэтому эксит нельзя, проверим потом
       }
