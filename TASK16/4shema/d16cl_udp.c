...
Fill_ip(sockaddr_in_server, ip, __LINE__);
  ...
  printf("NETWORK UDP Клиент запущен и устанавливает соединение с сервером по  сокету: %s %d\n",ip,ntohs(sockaddr_in_server.sin_port));
 
  
  while(1){
   printf("UDP клиент>Введите сообщение для сервера, ""exit"" - завершить программу\n");
   scanf("%s",msg_send);
...
CheckError(close(client_sock_fd), "close", __LINE__);   // чистим за собой и на выход
