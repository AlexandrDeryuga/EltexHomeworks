...
int main() {
  ...
  CheckError(client_sock_fd = socket(AF_LOCAL, SOCK_DGRAM, 0), "socket",__LINE__);   //(SOCK_DGRAM 0) - udp,  (SOCK_STREAM 0) - tcp
  printf("(1)UDP Клиент запущен на сокете: %s\n",sockaddr_un_client.sun_path);
  ...
  //CheckError(send(client_sock_fd, (char *)msg_send, STR_SIZE_MAX, 0), "send",__LINE__);  // пошлем серверу сообщение
    CheckError(sendto(client_sock_fd, (char *)msg_send, STR_SIZE_MAX, 0,(struct sockaddr *)&sockaddr_un_server,len), "send",__LINE__);  // пошлем серверу сообщение
