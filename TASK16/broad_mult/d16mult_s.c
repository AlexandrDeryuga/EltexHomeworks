...
int main() {
  ...
  sockaddr_in_server.sin_addr.s_addr = inet_addr("224.0.0.1"); // можно и так
...
//setsockopt(server_sock_fd , SOL_SOCKET, SO_BROADCAST, &flag, sizeof(flag));
  ...
  sprintf(msg_recv_send,"групповая рассылка от сервера"); //4. видоизменяем полученное сообщение и отправляем клиентам
 ...
 //CheckError(sendto(client_sock_fd, (char *)msg_send, STR_SIZE_MAX, 0,(struct sockaddr *)&sockaddr_in_server,len), "send",__LINE__);  // пошлем серверу сообщение
  ...
  printf("рассылаем сообщение группе: %s\n",msg_recv_send);
