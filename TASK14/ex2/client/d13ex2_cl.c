...
void wprintQmsg(WINDOW *wnd,myQmsg_type *Q)
{
wprintw(wnd, "q-print>, count=%d\n",Q->mcount);
for (int i=0;i<6;i++) 
 wprintw(wnd, "%d# type=%2ld prior=%2ld wait=%d  message>%s<\n",i,Q->SM_system[i].mType,Q->SM_system[i].priority,Q->SM_system[i].wait,Q->SM_system[i].mText);
...
char txtBuf[SYSBUF];
  ...
  wprintw(logw,"CLIENT: th1 -SystemMsg: started\n");wrefresh(logw);
    ...
    //pthread_mutex_lock( &mutex );
      msg.mType = 1;  //тип сообщений - "регистрация" от клиента [мой_pid Name]  
       ...
       //sprintf( d->Qsys.mText, "%lu Client[%lu]", (long)getpid(),(long)getpid());         // отправить запрос регистрации
       My_Pid= (long)getpid();    
 ...
 while(1){ // pthread_mutex_lock( &mutex ); 
           ...
           // pthread_mutex_unlock( &mutex ); 
          //if ( msgrcv( qId, &buf, sizeof msg.mText, msg.mType, 0 ) == -1 )   //заменить
          if ( Qsys_msgrcv( d->Qsys, &buf,1, 0.1, msg.mType ) < 0 )   
...
char Buf[BIGBUF];
  ...
  char *x;
    ...
    while (1){
       ...
       // wprintw(logw,"CLIENT th2> wait message on [%d]\n",msg.mType);wrefresh(logw); getch();
       // wprintw(logw,"_while1 %d   pointer=%ld\n",__LINE__,  (long)d->Qmsg); wrefresh(logw);//swhile (1); 
       //if ( msgrcv( qId2, &buf, sizeof msg.mText, msg.mType, 0 ) == -1 )   //заменить
        if ( Q_msgrcv( d->Qmsg, &buf,1, 0.1, msg.mType ) < 0 )
      {wprintw(logw,"ERROR <0 :CLIENT th2\n");wrefresh(logw);
      perror( "client: msgrcv failed:" );sleep(1);}
   else
      { 
   ...
   //   if(0==strcmp("send_last",buf.mText))// нет, пусть это сделает основной поток
      ...
      AddNewMessage(&Rooms[0],x,0,NULL,0); // засунем в указанную в соообщении комнату  наше сообщение, пока что все идет в общую
...
char Buf[SYSBUF];
  ...
  // key_t key_th1,key_th2; 
   ...
   myQmsg_type *Qmsg=NULL;
    ...
    //wprintw(logw,"_while1 %d   pointer=%ld\n",__LINE__,  (long)Qmsg); wrefresh(logw);//while (1); 
...
msg.mType = getpid();
  ...
  //wprintw(wtxt3,"mainwait..\n");
  //sleep(5);
  //wprintQ(wtxt1,Qsys);wrefresh(wtxt1);
  
  //getch();

 sprintf(msg.mText,"print_debug");  
        // if ( msgsnd( qId_th1, &msg, sizeof msg.mText, IPC_NOWAIT ) == -1 ) {
        if ( Qsys_msgsnd( Qsys, &msg, 0,0.1 ) == -1 ){
...
if(ch==pushmessage){
                        ...
                        ClientMessage[0]=0;
                         //if ( msgsnd( Qmsg, &msg, sizeof msg.mText, IPC_NOWAIT ) == -1 ) {             
                         if ( Q_msgsnd( Qmsg, &msg, 0,0.1 ) == -1 ){
                         perror( "server: msgsnd failed:" );
                         exit( 3 );
...
/////////////////////////////////////////////////
// завершение работы
   msg.mType = getpid();msg2.mType = getpid();
  sprintf(msg.mText,"exit_now"); sprintf(msg2.mText,"exit_now");   
        // if ( msgsnd( qId_th1, &msg, sizeof msg.mText, IPC_NOWAIT ) == -1 ) {
         if ( Qsys_msgsnd( Qsys, &msg, 0,0.1 ) == -1 ){
...
pthread_mutex_lock( &mutex );           /// RETURN не использовать изза мьютекса!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		{	case 9:		// Tab 
			...
			case 331:     //ins  отправить строку 
			        ...
			        if(chCount>0){InputString[chCount-3]=0; //срежем лишнее с инсерта
			                      strcpy(ClientMessage,InputString);
			        ...
			        wclear(wtxt);
                                wprintw(wtxt,"%s<",InputString); wrefresh(wtxt);
